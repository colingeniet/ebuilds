...
SRC_URI="mirror://sourceforge/flightgear/${P}-rc.tar.bz2"

LICENSE="GPL-2"
KEYWORDS="~amd64 ~x86"
SLOT="0"
IUSE="bash-completion compositor cpu_flags_x86_sse2 dbus debug examples +flite gdal openmp qt5 +udev +utils vim-syntax zsh-completion"
...
src_configure() {
	local mycmakeargs=(
		...
		-DENABLE_COMPOSITOR=$(usex compositor)
		-DENABLE_FGCOM=$(usex utils)
		-DENABLE_FGELEV=$(usex utils)
		-DENABLE_FGJS=$(usex utils)
		-DENABLE_FGVIEWER=$(usex utils)
		-DENABLE_FLITE=$(usex flite)
...
src_install() {
	...
	if use zsh-completion; then
		sed -e "s|/usr/local/share/FlightGear|${GAMES_DATADIR}/${PN}|" \
			-i scripts/completion/fg-completion.zsh || die 'unable to replace FG_ROOT'
		insinto /usr/share/zsh/site-functions
		doins scripts/completion/fg-completion.zsh
	fi
